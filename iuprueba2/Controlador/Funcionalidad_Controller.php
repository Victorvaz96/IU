...
session_start();
  	...
  	if(isset($_POST['funcionalidad']))
  		...
  		if (isset($_REQUEST['AltaFuncionalidad']))
  		{
			...
			$model->creador($nombrefuncionalidad, $descripcion);
  			...
  			if($resultado==FALSE)
  			{
				...
				//$vista->crear($idiom);
				$mensaje = new Mensaje();
				$mensaje ->crear($idiom, "Funcionalidad creada correctamente");
  			}else
  			{
				$alta=new funcionalidadAlta();
